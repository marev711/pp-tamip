 PROGRAM CMORIZETAMIP
     ...
     INTEGER :: error_flag, ncid, ncid2, rhVarId, timeVarId, &
                lonDimID, latDimId, timeDimId, levVarId, &
     ...
     DOUBLE PRECISION, dimension(:), allocatable :: alats, alons, time, levels
     DOUBLE PRECISION, dimension(:, :), allocatable :: alats_bounds, alons_bounds, time_bounds, levels_bounds
     ...
     call read_1d_coord(ncid, rhVarId, "lev", levels, levels_bounds)
     ...
     ilev = cmor_axis(table_entry='plev8',     &
                       units='Pa',       &
                       coord_vals=levels)
